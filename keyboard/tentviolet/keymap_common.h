...
/* TentVioletBoard keymap definition macro
 * only 13 columns and column 12, K0B is bound so that the last two keys are actually on their side
 * (minus, opensquare, quote, closesquare, plus): the last two go up. This is to save on pin usage because of ProMicro usage that only has 18 pins
 */
#define KEYMAP( \
